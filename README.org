...
- Level 3 :: attribute, assignment, constant, function, number,
  operator
...
- Command: ada-ts-mode-case-format-buffer ::
  ...
  =ada-ts-mode-case-formatting=.  If the buffer is narrowed, the
...
implementation of =ada-ts-case-category-p= should be created,
...
- User Option: ada-ts-project-root-markers ::
  File and directory identifiers which indicate the root of an Ada
  project.  These markers are used to determine the boundary of an
  Emacs (i.e., =project.el=) project.  If this value is set to nil, no
  special detection is provided for Ada projects.  The user can still
  configure ~project-vc-extra-root-markers~ manually to realize the same
  benefits, in such a case.  The intent of this option is to provide
  out-of-the-box settings to prevent or minimize the user from
  explicitly having to configure Ada project markers.
