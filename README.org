...
The following user option can be customized to modify the supported
LSP clients.

- User Option: ada-ts-mode-lsp-clients ::
  Provides a list of LSP clients for use by ~ada-ts-mode~.  This
  prevents the mode from depending directly on a specific LSP client.
  An entry is provided in the list for each supported LSP client.  If
  no LSP enhanced support is desired, either don't activate an LSP
  client in the buffer or set the list of LSP clients to nil.

To add an unsupported LSP client, an entry must be added to
~ada-ts-mode-lsp-clients~ and a set of methods must be implemented for
that entry which provide interfacing between ~ada-ts-mode~ and the
corresponding LSP client.  The methods must specialize on the client
parameter such that the entry in ~ada-ts-mode-lsp-clients~ can be used
to dispatch to the correct implementation.  The following are the list
of required methods:

- Function: ada-ts-mode-lsp-active-p client ::
  Determines whether the LSP client is active in the current buffer.

- Function: ada-ts-mode-lsp-command-execute client command arguments ::
  Execute Language Server command with optional arguments.

- Function: ada-ts-mode-lsp-command-supported-p client command ::
  Determines if Language Server supports command.

- Function: ada-ts-mode-lsp-document-id client ::
  Determine document identifier corresponding to current buffer.

- Function: ada-ts-mode-lsp-format-region client beg end ::
  Format using the Language Server for the specified region of the
  current buffer.
