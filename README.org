...
- Command: ada-ts-als-find-workspace-config-file ::
  ...
  found in the root of the LSP client defined workspace root
  directory, when an LSP client is active, otherwise the file is found
  relative to the project root as determined by project.el.  If the
  file does not exist, a new file will be visited.
